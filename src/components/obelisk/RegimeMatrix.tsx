        ...
        {regimes.map((row, i) => (
          regimes.map((col, j) => {
            ...
            return (
              <div 
                key={`${i}-${j}`}
                className={`p-4 rounded-2xl border flex flex-col items-center justify-center gap-1 transition-all ${isDiagonal ? "bg-black text-white border-black" : "bg-black/[0.02] border-black/[0.05]"}`}
              >
